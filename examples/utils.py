...
def draw_reconstucted_samples(
    ...
    figure = np.zeros((images_size[0] * num_row,
                       images_size[1] * (num_samples + 2)))
...
def q_IW(z, K=10, latent_dim=2, q_z=None, p_z=None):
    ...
    if p_z is None:
        ...
        def p_z(x): return np.log(
            np.mean(np.array([mix.pdf(x) for mix in mixture]), axis=0))

    z_latent = q_z.rvs(K).reshape([-1, latent_dim])
    z_latent[0] = z

    exponent = np.array([p_z(z_latent)]).reshape(
...
def draw_table(data, title, width=20):
    """
    Args:
    ...
    string += str(row_format.format("-" * width, *
                                    ["-" * width for _ in title])) + '\n'
    ...
    for key in data:
        if len(key) > width:
            ...
            row_name = key
        string += str(row_format.format(row_name, *
                                        [round(x, 5) for x in data[key]])) + '\n'
        string += str(row_format.format("-" * width, *
                                        ["-" * width for _ in title])) + '\n'
